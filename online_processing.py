...
def run_matchmaker(scenario_path, out_dir, p_ref_cache_dir, ref_start_time, method, hop_length = DEFAULT_HOP_LENGTH, sr = DEFAULT_SR, window_size = 10.0, distance_metric = 'cosine', step_size = None, readout = 'reduced'):
    '''
    ...
    readout -- str, 'reduced' or 'raw' (dixon only); raw writes alignment_path unreduced
    '''
...
def process_scenario(task):
    ...
    try:
        ...
        elif system == "MM-DIXON-RAW":
            # Same follower as MM-DIXON, reporting Dixon's alignment_path unreduced.
            # See matchmaker_worker.READOUTS.
            run_matchmaker(scenario_dir, out_dir, p_ref_cache_dir, ref_start_time, "dixon", hop_length=hop_length, sr=sr,
                           readout="raw")
        elif system == "MM-ARZT":
